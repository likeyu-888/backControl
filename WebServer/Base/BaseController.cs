    ...
    public class BaseController : ApiController
    {
        ...
        public BaseController()
        {
            ...
            if (!string.IsNullOrEmpty(token))
            {
                ...
                try
                {
                    ...
                    RedisHelper.Set(token, obj, tokenExpireSeconds);
        ...
        public decimal GetDecimal(string param, decimal defaultVal = 0)
        {
            if (jObject != null)
            {
                if (jObject.Property(param) != null)
                {
                    if (!DataValidate.IsNumber(jObject[param].ToString())) return defaultVal;
                    return Convert.ToDecimal(jObject[param]);
                }
            }
            if (requestForm.AllKeys.Contains(param))
            {
                if (!DataValidate.IsNumber(requestForm[param].ToString())) return defaultVal;
                return Convert.ToDecimal(requestForm[param]);
            }
            if (requestQuery.AllKeys.Contains(param))
            {
                if (!DataValidate.IsNumber(requestQuery[param].ToString())) return defaultVal;
                return Convert.ToDecimal(requestQuery[param]);
            }
            return defaultVal;
        }
        ...
        protected Hashtable GetDict(MySqlConnection conn, string dict)
        {
            return Dictionary.GetDict(conn, dict);
        }

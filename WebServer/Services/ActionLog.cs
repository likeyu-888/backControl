...
namespace Elite.WebServer.Services
{
    public class ActionLog
    {
        ...
        public static void Failed(MySqlConnection conn, long id, string remark = "")
        {
            List<MySqlParameter> parameters = new List<MySqlParameter>();
            parameters.Add(new MySqlParameter("@id", id));
            ...
            {
                commandText = "update log_action set status=0,remark=CONCAT(remark,'," + remark + "') where id=@id";
            }
            MysqlHelper.ExecuteNonQuery(conn, System.Data.CommandType.Text, commandText, parameters.ToArray());
        }


        public static long AddLog(MySqlTransaction transaction,
          int action_id,
          int device_id,
          int status,
          string username,
          ...
          )
        {
            ...
            long LastId = MysqlHelper.ExecuteNonQuery(transaction, System.Data.CommandType.Text, commandText, parameters.ToArray(), true);

            return LastId;
        }

        public static void Finished(MySqlTransaction transaction, long id)
        {
            List<MySqlParameter> parameters = new List<MySqlParameter>();
            parameters.Add(new MySqlParameter("@id", id));

            string commandText = "update log_action set status=1 where id=@id";
            MysqlHelper.ExecuteNonQuery(transaction, System.Data.CommandType.Text, commandText, parameters.ToArray());
        }



        public static void Failed(MySqlTransaction transaction, long id, string remark = "")
        {
            List<MySqlParameter> parameters = new List<MySqlParameter>();
            parameters.Add(new MySqlParameter("@id", id));

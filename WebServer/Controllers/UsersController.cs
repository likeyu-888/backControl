    ...
    public class UsersController : BaseController
    {
            ...
            )
        {
            ...
            using (conn = new MySqlConnection(Constr()))
            {
                ...
                String columns = "user.id, user.username, user.role_id, user.mobile, user.email, user.wechat, user.create_time, user.last_login_time, user.login_count, user.status*1 as status, val.text as role_text";

                StringBuilder commandText = new StringBuilder("select SQL_CALC_FOUND_ROWS " + columns +
                    "  from ucb_user user left join sys_config_value val on user.role_id=val.value and val.item_id=1002 where user.is_delete=0 ");
                ...
                {
                    commandText.Append(" and ((user.id = @keyword) or (user.username like CONCAT('%',@keyword,'%')) or (user.mobile like CONCAT('%',@keyword,'%')) or (user.email like CONCAT('%',@keyword,'%')))");
            ...
            )
        {
            ...
            using (conn = new MySqlConnection(Constr()))
            {
                ...
                String columns = "user.id, user.username, user.role_id, user.company, user.contact_name, user.mobile, user.email, user.wechat, user.create_time, user.last_login_time,user.last_login_ip, user.login_count, user.status*1 as status, val.text as role_text";

                StringBuilder commandText = new StringBuilder("select  " + columns +
                    "  from ucb_user  user left join sys_config_value val on user.role_id=val.value and val.item_id=1002 where user.id=@id and user.is_delete=0");
                List<MySqlParameter> parameters = new List<MySqlParameter>();
        ...
        public IHttpActionResult Put(JObject obj)
        {
            ...
            using (conn = new MySqlConnection(Constr()))
            {
                ...
                if (userInfo.id != id)
                {
                    if (userInfo.role_id != (int)RoleEnum.Admin)
                    {
                        return ErrorJson(403, "非法请求，未授权访问");
                    }
                }
        ...
        public IHttpActionResult UpdateStatus(JObject obj)
        {
            ...
            using (conn = new MySqlConnection(Constr()))
            {
                ...
                if (id == userInfo.id) return ErrorJson("不可修改自身状态");
        ...
        public IHttpActionResult UpdateRole(JObject obj)
        {
            ...
            if (id == userInfo.id) return ErrorJson("不可修改自身角色");
            ...
            using (conn = new MySqlConnection(Constr()))
            {
                ...
                string remark = "用户:" + row["username"].ToString() + ",角色：" + roleName;
                ...
                {
                    StringBuilder commandText = new StringBuilder();
                    commandText.Append("update ucb_user set role_id=@role_id where id=@id");

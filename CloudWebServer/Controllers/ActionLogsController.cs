            ...
            )
        {
            ...
            using (conn = new MySqlConnection(Constr()))
            {
                ...
                String columns = "log.id, log.create_time, log.username, log.status, (case log.status " +
                    ...
                    " when 1 then \"成功\" " +
                    " end) as status_text" +
                    ", log.ip, dev.ip as device_ip, log.remark, log.action_id,rule.name as action_text";

                StringBuilder commandText = new StringBuilder("select SQL_CALC_FOUND_ROWS " + columns +
                    "  from log_action log left join ucb_rule rule on   rule.id=log.action_id" +
                    " left join dev_device dev on dev.id=log.device_id  and dev.school_id=log.school_id" +
                    "  where 1=1 ");
                ...
                if (device_id != 0)
                {
                    commandText.Append(" and dev.cloud_id = @device_id");
